...
class AddPBBMap(QMainWindow, AddPBBMapDialog):
    ...
    def run(self):
        ...
        #conn = psycopg2._connect("dbname='db_pbb' host=localhost port=5433 user='postgres' password='septin'")
        #cur = conn.cursor()
        #cur.execute("""SELECT wa FROM gis.kecamatan order by wa""")
        #rows = cur.fetchall()
        #row_list = []
        #for row in rows:
            #row_list.append(row.)
        ...
        if result == 1:
            ...
            sql = "(SELECT * FROM gis.kecamatan where wa='TATANGA')"
            uri = QgsDataSourceURI()
            uri.setConnection("localhost", "5433", "db_pbb", "postgres", "septin")
            uri.setDataSource("",sql,"geom","","gid")
            tmadminkec = QgsVectorLayer(uri.uri(), "kecamatan", "postgres")
